...
- 오라클은 블록체인 외부에 있는 데이터를 트랜잭션에 등록해 동의를 얻은 후 블록체인 안으로 들여오고, 블록체인의 데이터를 외부로 내보내는 역할을 하며, 고립된 생태계인 블록체인을 외부와 연결 시켜주는 하나의 다리(Bridge) 역할을 함.
- 외부 데이터에 잘못된 정보가 섞여 있을 가능성이 있으므로 오라클을 제공하는 개인 또는 조직은 신뢰할 수 있어야 함.

<p align="center">
<img src="/assets/img/blog/oracle.png">
</p>

## 스마트 계약 - 오라클(Oracle) 문제

|하드웨어 오라클 문제(hardware oracle problem)|소프트웨어 오라클 문제(software oracle problem)
|:---:|:---:|
|- 센서(sensor)와 같은 하드웨어 기기를 사용하여 수집한 데이터를 블록체인 안에 기록할 때 발생하는 오라클 문제|- 소프트웨어를 사용하여 현실 세계의 데이터를 수집한 후 블록체인 안에 기록할 때 발생하는 오라클 문제|
|- 센서는 및, 소리, 온도, 압력 등 다양한 물리적인 환경 정보의 변화를 전기적 신호로 바꿔주는 역할을 함.|- 자료수집 봇(bot), 빅데이터 분석기, 인공지능(AI) 등 다양한 소프트웨어를 사용하여, 인터넷에 퍼져 있는 데이터를 수집하여 블록체인에 기록할 수 있음.|
|- 온도 센서, 습도 센서, 압력 센서, 지문 센서, 자이로 센서, 바이오 센서 등 다양한 센서에서 수집된 데이터를 블록체인 안에 자동으로 기록되게 할 수 있음.|- 예를 들어, 선거에서 A와 B 후보 중에서 누가 이겼는지 판단하기 위해, 인터넷 뉴스, 블로그, 트위터, 페이스북 등에 흩어져 있는 데이터를 수집한 후 인공지능을 사용하여 A 후보가 승리했다고 판단을 내리고, 블록체인에 전달해 줄 수 있음.|
|- 이 때 센서 장치의 오작동이나 부정확한 측정으로 인해 블록체인에 ㅣㄱ록되는 데이터가 정확하지 않을 수 있음.|- 만약 이러한 자료수집 봇이나 인공지능이 사실과 다른 판단을 내린다면, 블록체인 안에 기록된 데이터 역시 잘못된 정보가 됨.|
|- 블록체인 자체는 데이터의 위변조가 불가능하지만, 센서의 오작동으로 인해 블록체인에 기록되는 데이터가 부정확해져서 스마트 계약에 문제가 발생할 수 있음.|- 소프트웨어는 버그가 있을 수 있고, 악의적 해커에 의해 해킹을 당하여 오작동할 수도 있음.|
|- 이처럼 현실 세계와 블록체인 시스템의 중간에 위치하면서 현실 세계의 데이터를 수집하여 블록체인 시스템 안으로 입력해 주는 센서 등 하드웨어로 인해 발생하는 문제를 하드웨어 오라클 문제라고 함.|- 블록체인 자체는 해킹이 거의 불가능하지만, 블록체인에 데이터를 전달하는 SW는 상대적으로 쉽게 해킹을 당할 수 있음.|
||- 이처럼 데이터와 소프트웨어적인 문제로 인해 블록체인에 기록된 데이터가 부정확해지는 문제를 소프트웨어 오라클 문제라고 함.|

## 스마트 계약 - 오라클(Oracle)문제 해결방안

- 완벽한 오라클 모델을 만드는 것은 매우 어려운 일이지만 다양한 방법으로 오라클 모델을 만들고 있는 많은 프로젝트들이 존재함.
- 아직 완벽한 정답은 없지만 블록체인 오라클 문제를 해결하기 위해 투표, 중앙값, 중간자 등 다양한 방안이 제시되고 있음.

<p align="center">
<img src="/assets/img/blog/oracle_problem_solution.png">
</p>

## 스마트 계약 - 오라클(Oracle)문제 해결방안 - 중앙값

<p align="center">
<img src="/assets/img/blog/oracle_problem_solution_2.png">
</p>

## 스마트 계약(Smart contract) - 장점

<p align="center">
<img src="/assets/img/blog/smart_contract_advantages.png">
</p>

> 탈중앙화된 블록체인 인프라에서 계약을 실행하면 공격할 중앙 지점이 없고 뇌물을 줄 중앙 중개인도 없으며 어느 당사자나 중앙 관리자가 결과를 조작할 수 있는 메커니즘이 없음을 보장합니다.

> 계약 로직을 탈중앙화된 노드 네트워크에서 중복 처리하고 검증함으로써, 계약이 시간에 따라 조건에 따라 실행될 것임을 확실히 보장하는 강력한 방해 방지, 가동 시간 및 정확성 보증을 제공합니다.

> 탈중앙화된 네트워크를 사용하여 합의의 조건을 호스팅하고 시행함으로써, 이익을 추구하는 중개인이 특권적인 위치를 이용하여 가치를 착취하고 흡수하는 능력을 줄입니다.

> 합의의 백엔드 프로세스를 자동화함으로써-에스크로, 유지보수, 실행 및/또는 결제- 어느 당사자도 수동으로 데이터를 입력하고, 상대방이 자신의 의무를 이행하거나, 중개인이 거래를 처리하는 것을 기다릴 필요가 없게 됩니다.